...
let column_string_filter = function(datatables, input_name) {
    ...
    for(let tab_key in datatables) {
        ...
        if(typeof datatable.columns === 'function') {
            let _column_index = column_index(datatable, column_data)
            if(_column_index !== undefined) datatable.columns( _column_index ).search( $('.filter-' + column_data).val() )
        }
    }
...
let column_select_filter = function(datatables, input_name) {
    let column_data = get_column_data( input_name ).replace(/[[]]/,'')
    ...
    for(let tab_key in datatables) {
        ...
        if(typeof datatable.columns === 'function') {
            let val = $('.filter-' + column_data).val()
            if(val === 'all') val = ''
            let _column_index = column_index(datatable, column_data)
            if(_column_index !== undefined) datatable.columns( _column_index ).search(val)
        }
    }
}
...
let column_range_filter = function(datatables, input_name) {
    ...
    if(start_filter || end_filter) {
        ...
        for(let tab_key in datatables) {
            ...
            if(typeof datatable.columns === 'function') {
                let _column_index = column_index(datatable, column_data)
                if (_column_index) datatable.columns(_column_index).search(start_filter + " - " + end_filter)
            }
        }
    }
...
let column_filter = function(datatable, input_name, input_value) {
    ...
    if(typeof datatable.columns === 'function') {
        let _column_index = column_index(datatable, column_data)
        if(_column_index !== undefined) datatable.columns( _column_index ).search( input_value )
    }
}
...
let column_index = function(datatable, column_data) {
    let column_index = 0
    if(typeof datatable.column === 'function') {
        column_index = datatable.column('.col-' + column_data).index()
    }
    return column_index
}
...
let binding_filter_event_for = function (datatable) {
    ...
    $('button.reset-button').on('click', function(e){
        $('.datatable-filter-area input').val('')
        $('.datatable-filter-area select').val('').trigger('change.select2')
